...
- Each workflow-captured frozen order bundle is first regenerated byte-for-byte
  from its exact canonical planning input and strategy. The daily report is then
  independently rebuilt from those orders, normalized NBBO/trade files, replay
  specs, and order evidence before Phase 6 aggregation; re-hashed edits to
  orders, P&L, fills, or slippage are rejected.
